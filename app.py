...
# --- 1. CONFIG & AI AGENTS ---
st.set_page_config(page_title="Nifty Hedge Fund Master v6.0", layout="wide")
...
def summon_council(ticker, row, vix):
    ...
    # Using 1.5-flash for stable 2026 quota
    try:
        ...
        1. **The Bull:** Argue for the 'Aggressive Buy' or 'Mean Reversion' case.
        ...
        if "429" in str(e): return "🕒 Council is exhausted (Rate Limit). Please wait 60 seconds."
        return f"Council is currently in recess (Status: {e})"

# --- 2. THE NATIVE MATH ENGINE (ADX, Z-SCORE, ATR) ---
...
def calculate_zscore(series, window=20):
    """Mean Reversion: Standard Deviations from the mean"""
...
def run_master_scan(limit):
    ...
    all_data = []
    prog = st.progress(0, text="Executing Quantitative Multi-Strategy Scan...")
...
# --- 4. INTERFACE ---
st.title("🏹 Nifty Hedge Fund Master v6.0")
...
if 'master_results' in st.session_state:
    ...
    tabs = st.tabs(["🎯 Multi-Strategy Feed", "📈 Trend Ribbon", "🛡️ Risk Lab", "🧬 Intelligence Lab", "🗺️ Sector Heatmap"])
    
    with tabs[0]:
        st.subheader("Global Strategy Signal Feed")
        st.dataframe(df.sort_values("Miro_Score", ascending=False)[['Ticker', 'Signal', 'Miro_Score', 'Z-Score', 'Vol_Surge', 'Price']], use_container_width=True)
    ...
    with tabs[2]:
        ...
        if v_vix > 22: st.warning(f"⚠️ High VIX ({v_vix}): Delta-Neutral suggested. Hedge 40% with Nifty Puts.")
    ...
    with tabs[3]:
        ...
        if st.button("⚖️ Summon Council Debate"):
            with st.spinner(f"Debating {target}..."):
                st.markdown(summon_council(target, df[df['Ticker'] == target].iloc[0], v_vix))

    with tabs[4]:
        st.subheader("Sector Relative Strength (Average Miro Score)")
        sector_perf = df.groupby('Sector')['Miro_Score'].mean().sort_values(ascending=False)
